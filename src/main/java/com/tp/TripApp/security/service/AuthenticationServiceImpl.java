...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public AuthenticationResponse register(RegisterRequest request, MultipartFile fPermis, 
        ...
        List<? extends GrantedAuthority> roles = user.getAuthorities()
                .stream()
                .toList();
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request) {
        ...
        CompteUtilisateur user = userRepository.findByTelephoneOrLogin(request.getUsername(), request.getUsername()).orElseThrow(() -> new IllegalArgumentException("Invalid 	or password."));
        var jwt = jwtService.generateToken(user);
        var refreshToken = refreshTokenService.createRefreshToken(user.getId());
        List<? extends GrantedAuthority> roles = user.getAuthorities()
                .stream()
